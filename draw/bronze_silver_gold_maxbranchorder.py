...
max_branch_order_column = 'Q'

# exp2_file_path = r'C:\Users\penglab\Documents\金银铜\autoQC_数据生产流程.xlsx'
# exp2_sheet_name = "流程一"
# exp2_column = 'N'

def get_max_branch_order(file_path, sheet_name, max_branch_order_column):
    # 打开 Excel 文件
    wb = load_workbook(file_path)
    sheet = wb[sheet_name]
    data = []
    for cell in sheet[max_branch_order_column][1:]:
...
bronze_data = get_max_branch_order(bronze_file_path, bronze_sheet_name, max_branch_order_column)
silver_data = get_max_branch_order(silver_file_path, silver_sheet_name, max_branch_order_column)
gold_data = get_max_branch_order(gold_file_path, gold_sheet_name, max_branch_order_column)
...
# 显示图形
# plt.title("MaxBranchOrder of the bronze standard, silver standard and gold standard", fontsize=18, pad=20)
plt.ylabel("MaxBranchOrder", fontsize=20)
plt.xlabel('')
plt.show()
