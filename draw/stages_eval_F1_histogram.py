...
auto_data, bronze_data, silver_data, gold_data, ground_truth_data = get_metric(synthetic_15_file_path, synthetic_15_sheet_name, synthetic_auto_F1_column, synthetic_bronze_F1_column, synthetic_silver_F1_column, synthetic_gold_F1_column, is_human=False)
...
data = [np.copy(bronze_data), np.copy(silver_data)]
# 计算标准差
std_devs = [np.std(group) for group in data]
# 计算标准误差
# std_errs = [stats.sem(group) for group in data]
data_avg = [np.mean(bronze_data), np.mean(silver_data)]
print(data_avg)
...
colors = ['#e4bd95', '#70b092']
# 手动计算每个柱子的位置
x = np.arange(len(types)) * (bar_width + spacing)
print(x)

bars = plt.bar(x, data_avg, width=bar_width, color=colors)
# 添加误差条
plt.errorbar(x, data_avg, yerr=errors, fmt='none', ecolor='white', capsize=8, elinewidth=2)  # 设置误差条颜色为白色
...
# 更改刻度颜色
plt.tick_params(colors='white')  # 刻度颜色
# 获取当前 Axes 对象
ax = plt.gca()
# 设置坐标轴颜色为白色
for spine in ax.spines.values():
    spine.set_color('white')  # 更改坐标轴线条颜色
# 设置绘图区背景颜色为黑色
